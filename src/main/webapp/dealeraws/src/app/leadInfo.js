...
var Leads = (function () {
    ...
    Leads.prototype.goto = function (item, type) {
    ...
    Leads.prototype.getleads = function (l_dealerId) {
        var _this = this;
        try {
            this.shareservice.showLoader();
            this._service.getleads(l_dealerId).subscribe(function (data) {
                if (data != undefined) {
                    _this.shareservice.hideLoader();
                    var l_response = JSON.parse(data['_body']);
                    _this.leads = l_response;
                }
            }, function (err) {
                _this.shareservice.hideLoader();
                console.log(err);
    ...
    Leads.prototype.getquotes = function (leadid, l_dealerId) {
        var _this = this;
        try {
            this.shareservice.showLoader();
            this._service.getquotes(leadid, l_dealerId).subscribe(function (data) {
                if (data != undefined) {
                    _this.shareservice.hideLoader();
                    var l_response = JSON.parse(data['_body']);
                    _this.leads = l_response;
